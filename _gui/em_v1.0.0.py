import sys
import string
from PyQt5.QtWidgets import QApplication, QWidget, QVBoxLayout, QHBoxLayout, QLabel, QLineEdit, QPushButton, QTextEdit, QMessageBox, QSpinBox, QTabWidget, QRadioButton, QButtonGroup
...
class EnigmaGUI(QWidget):
    ...
    def initUI(self):
        self.setWindowTitle('Enigma Machine - MSHS')
        ...
        # Custom reflector choice
        reflector_layout = QHBoxLayout()
        reflector_label = QLabel('Use Custom Reflector:')
        reflector_label.setFont(font)
        reflector_layout.addWidget(reflector_label)

        self.yes_radio = QRadioButton("Yes")
        self.no_radio = QRadioButton("No")
        self.no_radio.setChecked(True)

        reflector_layout.addWidget(self.yes_radio)
        reflector_layout.addWidget(self.no_radio)

        self.reflector_button_group = QButtonGroup()
        self.reflector_button_group.addButton(self.yes_radio)
        self.reflector_button_group.addButton(self.no_radio)

        # Custom reflector input box
        self.custom_reflector_input = QLineEdit()
        self.custom_reflector_input.setFont(font)
        self.custom_reflector_input.setPlaceholderText("Enter 26 unique uppercase letters")
        self.custom_reflector_input.setEnabled(False)  # Disabled by default

        reflector_layout.addWidget(self.custom_reflector_input)
        main_layout.addLayout(reflector_layout)

        # Connect the radio buttons to a method that handles enabling/disabling the input box
        self.yes_radio.toggled.connect(self.toggle_custom_reflector_input)
    ...
    def toggle_custom_reflector_input(self):
        if self.yes_radio.isChecked():
            self.custom_reflector_input.setEnabled(True)
        else:
            self.custom_reflector_input.setEnabled(False)
    ...
    def get_reflector(self):
        if self.yes_radio.isChecked():
            custom_wiring = self.custom_reflector_input.text().strip().upper()
            if len(custom_wiring) == 26 and set(custom_wiring) == set(string.ascii_uppercase):
                return Reflector(custom_wiring)
            else:
                QMessageBox.warning(self, "Input Error", "Custom reflector wiring must consist of 26 unique uppercase letters.")
                return None
        else:
            return Reflector("YRUHQSLDPXNGOKMIEBFZCWVJAT")  # Default reflector
